...
class VersionedTest implements Versioned {
  ...
  public static fromBytes(bytes: Uint8Array): VersionedTest {
    return VersionedParser.fromVersionedBytes(
      VersionedTest.getVersionHandler(),
      bytes
    );
  }
  ...
  protected static getVersionHandler(): VersionHandler {
    const oldVersionDeserializers = (): VersionedDeserializers => {
      ...
      const deserializerV10 = (bytes: Uint8Array) =>
        new VersionedTest(bytes.slice(1));
...
describe('versioned', () => {
  it('serializes and deserialized current version', () => {
    ...
    expect(vt.toBytes()).toEqual(
      new Uint8Array([84, 101, 115, 116, 0, 2, 0, 0, 0, 1, 2, 3])
    );
  ...
  it('serializes and deserialized previous version', () => {
    ...
    const vt = new VersionedTest(payload);
    expect(vt.toBytes()).toEqual(
      new Uint8Array([84, 101, 115, 116, 0, 1, 0, 0, 0, 1, 2, 3])
    );

...
describe('blockchain policy', () => {
  describe('calculates policy value', () => {
    ...
    it('rejects missing negative parameters', () => {
      ...
      expect(call).toThrow(
        `Negative policy parameters are not allowed: shares is ${shares}`
      );
    });
    ...
    it('rejects missing value and rate', () => {
      ...
      const call = () =>
        BlockchainPolicy.calculateValue(1, 1, undefined, undefined);
      expect(call).toThrow(
    ...
    it('rejects value not divisible by the number of shares', () => {
      ...
      const value = 1;
      const call = () =>
        BlockchainPolicy.calculateValue(shares, 1, value, undefined);
      expect(call).toThrow(
    ...
    it('rejects value per node not divisible by the number of periods', () => {
      ...
      const value = 1;
      const call = () =>
        BlockchainPolicy.calculateValue(1, paymentPeriods, value, undefined);
      expect(call).toThrow(
        `Policy value of ${value} wei per node cannot be divided by duration ` +
